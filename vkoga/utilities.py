...
class OptimizedKernel(torch.nn.Module):
    ...
    def __init__(self, kernel, dim,
                 reg_para=1e-5, learning_rate=1e-3, n_epochs=100, batch_size=64, n_folds=None,
        ...
        #self.A = nn.Parameter(torch.tensor([[3, 0.0], [0.0, 1]]), requires_grad=True)
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=250, gamma=.7)
    ...
    def linear_kernel_eval(self, X, Z):
        return X @ Z.t()
    ...
    def optimize(self, X, y, flag_optim_verbose=True):
        ...
        for idx_epoch in range(self.n_epochs):
            ...
            for idx_batch in range(n_batches):
                ...
                # kernel_matrix = self.linear_kernel_eval(Xb @self.A, Xb @self.A)
